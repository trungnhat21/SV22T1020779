...
public static class PartnerDataService
{
    ...
    static PartnerDataService()
    {
        ...
        accountDB = new CustomerAccountRepository(Configuration.ConnectionString);
    }
    ...
    public static async Task<Shipper?> GetRandomShipperAsync()
    {
        var input = new PaginationSearchInput { Page = 1, PageSize = 100, SearchValue = "" };
        var result = await ListShippersAsync(input);
        var list = result.DataItems; //
